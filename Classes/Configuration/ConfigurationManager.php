 ...
 * @version $ID:$
 */
class Tx_Palm_Configuration_ConfigurationManager implements Tx_Palm_Configuration_ConfigurationManagerInterface {
	...
	protected function initializeConcreteConfigurationManager() {
		if (TYPO3_MODE === 'FE') {
			$this->concreteConfigurationManager = $this->objectManager->get('Tx_Palm_Configuration_FrontendConfigurationManager');
		} else {
			$this->concreteConfigurationManager = $this->objectManager->get('Tx_Palm_Configuration_BackendConfigurationManager');
		}
	}
	...
	public function getConfiguration($configurationType, $extensionName = NULL, $pluginName = NULL) {
		switch ($configurationType) {
			...
			default :
				throw new Tx_Extbase_Configuration_Exception_InvalidConfigurationTypeException('Invalid configuration type "' . $configurationType . '"', 1302513786);
		}
	}
